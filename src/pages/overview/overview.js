...
app.controller('OverViemController', ['$scope', '$http', '$state', '$modal', '$rootScope', function ($scope, $http, $state, $modal, $rootScope) {
    ...
    $scope.sysState = function (params) {
        $http({
            ...
            if (data.status == 0) {
                ...
                // 弹窗内容拓扑图
                $scope.show_detail = function (size) {
        ...
        $http({
            ...
            if (data.status == 0) {
                ...
                myChart.on('click', function (param) {
                    if ($rootScope.menuList.indexOf('12') > 0) {
                        if (param.data.name == '高') {
                            $state.go('app.alarm', {
                                data: 'high'
                            });
                        }
                        if (param.data.name == '中') {
                            $state.go('app.alarm', {
                                data: 'medium'
                            });
                        }
                        if (param.data.name == '低') {
                            $state.go('app.alarm', {
                                data: 'low'
                            });
                        }
    ...
    // 跳转到告警详情页面 
    $scope.goAlarmDetail = function (item) {
        if ($rootScope.menuList.indexOf('12') > 0) {
    ...
    $scope.risk_property = function () {
        $http({
            ...
            if (data.status == 0) {
                $scope.risk_property_data = data.data;
                angular.forEach($scope.risk_property_data, function (item, index) {
                    item.style = {
    ...
    $scope.goAlarmRisk = function () {
        if ($rootScope.menuList.indexOf('20') > 0) {
    ...
    $scope.graph_echart = function () {
        ...
        if ($scope.only_to) {
            ...
            $scope.data_item0 = {
                ...
                names: "引擎",
                dev_ip: $scope.engine_array[0]
                    .dev_ip,
                status: $scope.engine_array[0]
                    .status,
                //节点上面的文字
                label: {
                    normal: {
